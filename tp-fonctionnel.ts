...
let firstAdult: { name: string; age: number } | null = null;
...
/*
Exercice 5.5 :
Trier les utilisateurs par âge et récupérer les deux plus jeunes
*/

// Copier le tableau
// Exercice 5.5 : trier par âge et récupérer les deux plus jeunes

type UserSimple = { name: string; age: number };

// Copier le tableau (sans modifier l’original)
const usersCopy: UserSimple[] = [];
for (let i = 0; i < users.length; i = i + 1) {
...
for (let i = 0; i < usersCopy.length; i = i + 1) {
  for (let j = 0; j < usersCopy.length - 1; j = j + 1) {
    if (usersCopy[j].age > usersCopy[j + 1].age) {
      const temp: UserSimple = usersCopy[j];
...
const twoYoungest: UserSimple[] = [usersCopy[0], usersCopy[1]];
